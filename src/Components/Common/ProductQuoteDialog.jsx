...
const ProductQuoteDialog = ({ product, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#101a2c]/75 p-3 backdrop-blur-sm sm:p-6" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div role="dialog" aria-modal="true" aria-labelledby="quotation-dialog-title" className="relative grid max-h-[calc(100dvh-24px)] w-full min-w-0 max-w-[940px] overflow-y-auto rounded-[18px] bg-white shadow-[0_24px_80px_rgba(0,0,0,.28)] sm:max-h-[94dvh] sm:rounded-[22px] md:grid-cols-[1.05fr_.95fr]">
