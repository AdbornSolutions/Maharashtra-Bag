...
const Navbar = ({ variant = 'default' }) => {
  ...
  return (
    <header className={`${isTransparent ? 'absolute inset-x-0 top-0 border-b border-white/10 bg-black/20 text-white' : isLight ? 'relative border-b border-[#e4e7eb] bg-white text-[#17243b]' : 'sticky top-0 border-b border-white/10 bg-black text-white'} z-50 w-full px-4 py-3 sm:px-8 lg:px-[5.3%] lg:py-5`}>
      <nav className="mx-auto flex max-w-[1260px] items-center justify-between gap-5" aria-label="Main navigation">
        ...
        <div className="hidden flex-1 items-center justify-end gap-7 lg:flex xl:gap-8">
          ...
          <a href="/contact" className="rounded-full bg-[#1c3663] px-9 py-4 text-sm font-bold transition-colors hover:bg-[#28477e]">
            Get a Quote
          </a>
          <a href="/contact" className="flex size-[52px] items-center justify-center rounded-full bg-[#ff7900] transition-transform hover:scale-105" aria-label="Get a quote">
        ...
        <div className="flex items-center gap-3 lg:hidden">
          ...
          <button
            ...
            className={`rounded-md border px-3 py-2 text-sm font-semibold ${isLight ? 'border-[#17243b]/30' : 'border-white/30'}`}
      ...
      {isMenuOpen && (
        <div id="mobile-navigation" className={`mx-auto mt-3 max-w-[1260px] border-t pt-3 lg:hidden ${isLight ? 'border-[#17243b]/15 bg-white' : 'border-white/15'}`}>
